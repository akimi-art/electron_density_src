...
savepath = os.path.join(current_dir, "results/figure/sii_luminosity_vs_sm_sfr_COMPLETE.png")
...
# # ============================================================
# # 追加図：
# #   L([S II] 6716) vs log(sSFR)
# # ============================================================

# out_png_ssfr = os.path.join(
#     current_dir,
#     "results/figure/sii6716_luminosity_vs_ssfr_COMPLETE.png"
# )
...
# x_ssfr = log_sSFR[m_ssfr_6716]
# y_L6716 = L6716[m_ssfr_6716]
...
# ax.set_ylabel(r"$L(\mathrm{[S\,II]}\ 6716)\ [\mathrm{erg\ s^{-1}}]$")

# ax.set_xlim(-14, -7)        # 必要に応じて調整
...
# plt.tight_layout()
# plt.savefig(out_png_ssfr, dpi=200)
# plt.show()

# print("Saved:", out_png_ssfr)
...
# SFR 欠損（-1.0）→ NaN
logSFR = np.where(logSFR == -1.0, np.nan, logSFR)
...
dL[valid_z] = cosmo.luminosity_distance(z[valid_z]).to(u.cm).value
L6716 = 4 * np.pi * dL**2 * F6717  # [erg s^-1]
...
mask_M    = np.isfinite(logM)    & m_Lpos
mask_SFR  = np.isfinite(logSFR)  & m_Lpos
mask_sSFR = np.isfinite(log_sSFR) & m_Lpos
...
# --- L6716 vs logM* ---
ax1.scatter(logM[mask_M], L6716[mask_M], s=1, alpha=0.6)
ax1.set_xlim(8, 12)
ax1.set_ylabel("L([S II] 6716) [erg/s]")
ax1.set_xlabel(r"$\log(M_\ast/M_\odot)$")

# --- L6716 vs logSFR ---
ax2.scatter(logSFR[mask_SFR], L6716[mask_SFR], s=1, alpha=0.6)
ax2.set_xlim(-3, 3)
ax2.set_xlabel(r"$\log(\mathrm{SFR})\ [M_\odot\,\mathrm{yr}^{-1}]$")
ax2.tick_params(labelleft=False)

# --- L6716 vs log(sSFR) ---
ax3.scatter(log_sSFR[mask_sSFR], L6716[mask_sSFR], s=1, alpha=0.6)
ax3.set_xlim(-14, -7)
...
savepath = os.path.join(
    current_dir,
    "results/figure/sii6716_luminosity_vs_sm_sfr_ssfr_COMPLETE.png"
)
plt.savefig(savepath, dpi=200, bbox_inches="tight")
print(f"Saved: {savepath}")
plt.show()