...
"""
スクリプトの概要:
このスクリプトはne_vs_metallicityのプロットデータの
統計検定を行います。
...
Samir16out     = os.path.join(current_dir, "results/Samir16/Samir16out_standard_v4_metal_v1_no_agn.py")
...
# === inputファイルから情報を抜き出す ===
Samir16in     = os.path.join(current_dir, "results/Samir16/Samir16in_standard_v4_metal_v1_no_agn.txt")
...
with open(Samir16in, "r") as f:
    for i, line in enumerate(f):
        # if i >= 0: # 現時点でまだSDSSのmetallicityの情報は載せていない
        #     break
...
# ============================================================
x_data = np.array(metal_list)         # x = log10(M*)
y_data = np.array(ne_list)            # y = log10(ne)
xerr_data = np.array(metal_xerr_list) # x の誤差（dex）
yerr_data = np.array(ne_yerr_list)    # y の誤差（dex）t
...
# ============================================================
# --- 有効分散を用いた対数尤度 ---
#     sigma_eff^2 = yerr^2 + (a * xerr)^2
# ============================================================
def log_likelihood(theta, x, y, xerr, yerr):
    a, b = theta
    y_model = a * x + b
    sigma2 = yerr**2 + (a * xerr)**2
...
initial_guess = np.array([0.1, 0.0])
...
if len(x_m) < 2:
    ...
    # x の代表グリッド（将来の描画用）
    x_band = np.linspace(6.5, 9.5, 200)
    
    # posterior から y(x) を生成
    y_band_samples = np.array([
        a * x_band + b for a, b in flat_samples
    ])
    
    # 各 x に対する統計量
    y_band_med  = np.percentile(y_band_samples, 50, axis=0)
    y_band_low  = np.percentile(y_band_samples, 16, axis=0)
    y_band_high = np.percentile(y_band_samples, 84, axis=0)
    ...
    band_df.to_csv("results/csv/ne_vs_metallicity_regression_band.csv", index=False)
    print("[INFO] Regression band saved to ne_vs_metallicity_regression_band.csv")

    a_mcmc, b_mcmc = np.mean(flat_samples, axis=0)
    a_std, b_std = np.std(flat_samples, axis=0)

    print(f"MCMC slope (a):      {a_mcmc:.3f} ± {a_std:.3f}")
    print(f"MCMC intercept (b):  {b_mcmc:.3f} ± {b_std:.3f}")
    print(f"{tau:.3f}, {p_value:.3f}, {a_mcmc:.3f}, {a_std:.3f}, {b_mcmc:.3f}, {b_std:.3f}")

    # ========================================================
    # --- フィットの可視化 ---
    # ========================================================
    x_fit = np.linspace(6.5, 9.5, 500)
...
plt.tight_layout()
plt.savefig(os.path.join(current_dir, "results/figure/ne_vs_metallicity_kendall_z~9_low_ion"))
plt.show()
...
print(np.min(metal_list), np.max(metal_list))
