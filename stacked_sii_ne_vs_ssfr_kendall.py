...
"""
スクリプトの概要:
このスクリプトはne_vs_ssfrのスタックデータの
統計検定を行います。

使用方法:
    stacked_sii_ne_vs_ssfr_kendall.py [オプション]
...
# x,y（log空間）
x_data = df["logsSFR_cen"].to_numpy(float)
y_data = df["log_ne_med"].to_numpy(float)

# xerr：ビン幅/2（なければ固定値でもOK）
xerr_data = 0.5 * (df["logsSFR_hi"].to_numpy(float) - df["logsSFR_lo"].to_numpy(float))
