...
"""
スクリプトの概要:
このスクリプトはne_vs_sfrのプロットデータの
統計検定を行います。

使用方法:
    ne_vs_sfr_kendall.py [オプション]
...
# ============================================================
x_data = np.array(sfr_list)            # x = log10(M*)
y_data = np.array(ne_list)            # y = log10(ne)
xerr_data = np.array(sfr_xerr_list)    # x の誤差（dex）
yerr_data = np.array(ne_yerr_list)    # y の誤差（dex）
...
if len(x_m) < 2:
    ...
    band_df.to_csv("results/csv/ne_vs_sfr_regression_band_.csv", index=False)
    print("[INFO] Regression band saved to ne_vs_sfr_regression_band.csv")
...
plt.tight_layout()
plt.savefig(os.path.join(current_dir, "results/figure/ne_vs_sfr_plot_sdss_ms.png"))
plt.show()
...
print(np.min(sfr_list), np.max(sfr_list))
