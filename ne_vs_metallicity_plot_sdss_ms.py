...
Samir16out    = os.path.join(current_dir, "results/Samir16/Samir16out_standard_v3_ms_only_v3_merged_direct_Te.py")
...
# === inputファイルから情報を抜き出す ===
Samir16in     = os.path.join(current_dir, "results/Samir16/Samir16in_standard_v3_ms_only_v3_merged_direct_Te.txt")
...
with open(Samir16in, "r") as f:
    for i, line in enumerate(f):
        # if i >= 100: # 現時点でまだSDSSのmetallicityの情報は載せていない
        #     break
...
plt.text(x=6.5+0.1, y=np.log10(1917.5607046610592)+0.1, s=r"$n_{\mathrm{crit}}$([SII]6716)", fontsize=12,)
...
plt.tight_layout()
plt.savefig(os.path.join(current_dir, "results/figure/ne_vs_metallicity_plot_sdss_ms_slope_sdss_no_agn_direct_editted.png"))
plt.show()
