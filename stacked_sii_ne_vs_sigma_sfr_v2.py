...
使用方法:
    stacked_sii_ne_vs_sigma_sfr_v2.py [オプション]
...
out_csv = os.path.join(current_dir, "results/csv/stacked_sii_ratio_vs_sigma_sfr_COMPLETE_v2.csv")
out_png = os.path.join(current_dir, "results/figure/stacked_sii_ratio_vs_sigma_sfr_COMPLETE_v2.png")
...
for lo, hi in zip(edges[:-1], edges[1:]):
    ...
    N = np.sum(sigma_sfr_bin)
    if N < NMIN:
        continue

    f1 = F6716[sigma_sfr_bin]
    e1 = err6716[sigma_sfr_bin]
    f2 = F6731[sigma_sfr_bin]
    e2 = err6731[sigma_sfr_bin]
    fHa = FHa[sigma_sfr_bin]
    eHa = errHa[sigma_sfr_bin]
...
save_path_count = os.path.join(
    fig_dir,
    "heat_simga_sfr_sii_ratio_count_sdss.png"
)
...
hist_path = os.path.join(
    current_dir,
    "results/figure/stacked_sii_histograms_sigma_sfr.png"
)
