...
# ============================================================
x_data = np.array(ssfr_list)            # x = log10(M*)
y_data = np.array(ne_list)              # y = log10(ne)
xerr_data = np.array(ssfr_xerr_list)    # x の誤差（dex）
...
if len(x_m) < 2:
    ...
    band_df.to_csv("results/csv/ne_vs_ssfr_regression_band.csv", index=False)
    print("[INFO] Regression band saved to ne_vs_ssfr_regression_band.csv")
...
print(np.min(ssfr_list), np.max(ssfr_list))
