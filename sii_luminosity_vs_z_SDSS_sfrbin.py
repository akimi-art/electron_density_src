...
使用方法:
    sii_luminoisity_vs_z_SDSS_sfrbin.py [オプション]
...
fits_path = os.path.join(
    current_dir,
    "results/fits/mpajhu_dr7_v5_2_merged.fits"  # もしくは絞り込み済みfitsへのパス
)
t = Table.read(fits_path, format="fits")
df = t.to_pandas()

# === 列の読み込み ===
UNIT_FLUX = 1e-17  # MPA-JHU の線フラックスは 1e-17 cgs 単位
z       = df["Z"].to_numpy(float)
logSFR  = df["sfr_MEDIAN"].to_numpy(float)  # log(SFR/Msun/yr)
...
# === 欠損（-1.0）を NaN に置換（★重要：ビン境界に-1が含まれるため、欠損の -1.0 は除外）
missing_sfr_value = -1.0
logSFR = np.where(logSFR == missing_sfr_value, np.nan, logSFR)

# === S/N（安全計算：err>0 でのみ計算、他は NaN）
with np.errstate(divide='ignore', invalid='ignore'):
    sn6716 = np.divide(F6716, err6716, out=np.full_like(F6716, np.nan),
                       where=np.isfinite(err6716) & (err6716 > 0))
    sn6731 = np.divide(F6731, err6731, out=np.full_like(F6731, np.nan),
                       where=np.isfinite(err6731) & (err6731 > 0))

# === 光度（L = 4π d_L^2 F）
dL = np.full_like(z, np.nan, dtype=float)
m_dz = np.isfinite(z) & (z >= 0)
dL[m_dz] = cosmo.luminosity_distance(z[m_dz]).to(u.cm).value
L6716 = 4 * np.pi * dL**2 * F6716
L6731 = 4 * np.pi * dL**2 * F6731

# === SFRビン ===
# [-3,-2), [-2,-1), [-1,0), [0,1), [1,2), [2,3]（最後のビンのみ上限含む）
sfr_edges = np.array([-3, -2, -1, 0, 1, 2, 3], dtype=float)
n_cols = len(sfr_edges) - 1
...
# カラーマップ（S/N着色。カラーバーは付けない）
norm = TwoSlopeNorm(vcenter=0, vmin=-3, vmax=5)
cmap = "coolwarm"

# === 図（GridSpec 2×6：隙間ゼロ、タイトルなし、カラーバーなし、凡例でビン注記） ===
fig = plt.figure(figsize=(20, 6))
gs  = gridspec.GridSpec(2, n_cols, figure=fig, wspace=0.0, hspace=0.0)

for j in range(n_cols):
    slo, shi = sfr_edges[j], sfr_edges[j+1]
    # 最後のビンのみ上限含む
    if j < n_cols - 1:
        in_bin = (logSFR >= slo) & (logSFR < shi)
        bin_label = f"{slo:.0f} ≤ logSFR < {shi:.0f}"
    else:
        in_bin = (logSFR >= slo) & (logSFR <= shi)
        bin_label = f"{slo:.0f} ≤ logSFR ≤ {shi:.0f}"
    ...
    ax1.scatter(z[mask6716], L6716[mask6716],
                c=sn6716[mask6716], cmap=cmap, norm=norm, s=10, alpha=0.75)
    # 一定フラックス線
    for i, F0 in enumerate(flux_lines):
        ax1.plot(z_grid, Lfl(F0), color="black", lw=1.0,
                 ls=["--", "-.", ":"][i % 3], alpha=0.9)
    ...
    # ★ 凡例でビン注記（タイトルは付けない）
    ax1.plot([], [], ' ', label=bin_label)
    leg1 = ax1.legend(loc='lower right', frameon=True, fontsize=10,
                      handlelength=0, handletextpad=0.2, edgecolor='black')
    ...
    ax2.scatter(z[mask6731], L6731[mask6731],
                c=sn6731[mask6731], cmap=cmap, norm=norm, s=10, alpha=0.75)
    for i, F0 in enumerate(flux_lines):
        ax2.plot(z_grid, Lfl(F0), color="black", lw=1.0,
                 ls=["--", "-.", ":"][i % 3], alpha=0.9)
    ...
    # ★ 下段にも凡例（同じテキスト）
    ax2.plot([], [], ' ', label=bin_label)
    leg2 = ax2.legend(loc='lower right', frameon=True, fontsize=10,
                      handlelength=0, handletextpad=0.2, edgecolor='black')
...
# === 保存 ===
save_path = os.path.join(current_dir, "results/figure/mpajhu_sfrbin_2x6_L6716_L6731.png")
