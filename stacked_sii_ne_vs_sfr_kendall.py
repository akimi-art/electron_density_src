...
"""
スクリプトの概要:
このスクリプトはne_vs_sfrのスタックデータの
統計検定を行います。

使用方法:
    stacked_sii_ne_vs_sfr_kendall.py [オプション]
...
in_csv  = "results/csv/stacked_sii_ne_vs_sfr_from_ratio_COMPLETE_v1.csv"
band_csv = "results/csv/stacked_ne_vs_sfr_regression_band_v1.csv"
out_png = "results/figure/stacked_ne_vs_sfr_v1.png"
...
# x,y（log空間）
x_data = df["logSFR_cen"].to_numpy(float)
y_data = df["log_ne_med"].to_numpy(float)

# xerr：ビン幅/2（なければ固定値でもOK）
xerr_data = 0.5 * (df["logSFR_hi"].to_numpy(float) - df["logSFR_lo"].to_numpy(float))
